...
import { add, rm, rmAll, addAll } from '../redux/NumProducts'
import { push, remove, addTotal, rmTotal, eqTotal } from '../redux/ShopSlice';
    ...
    return (
        <div>
            {prdNum === 0 || window.localStorage.getItem(`remove-${user.id}`) ? (
                <button className='bg-sky-600 text-white p-3 rounded-md w-full' onClick={addPrd}>
                    Add To Cart
                </button>
            ) : (
                <div className='text-center'>
                    <div className='flex flex-row justify-center items-center m-2'>
                        <button className='bg-sky-600 text-white py-1 px-3 rounded-md w-fit' onClick={rmPrd}>
                            -
                        </button>
                        <div className='m-2'>{prdNum} in cart</div>
                        <button className='bg-sky-600 text-white py-1 px-3 rounded-md w-fit' onClick={addPrd}>
                            +
                        </button>
                    </div>
                    <button className='bg-red-500 text-white p-3 rounded-md w-full' onClick={rmAl}>
                        Remove
                    </button>
                </div>
            )}
